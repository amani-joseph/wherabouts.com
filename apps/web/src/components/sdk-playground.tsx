...
import { EMPTY_SCENE, type MapScene } from "./sdk-playground/map-scene";
import {
	sceneFromInputs,
	sceneFromResult,
} from "./sdk-playground/scene-builders";
import { SdkResultMap } from "./sdk-playground/sdk-result-map";
...
export function SdkPlayground() {
	...
	const inputScene = useMemo(
		() => (endpoint ? sceneFromInputs(endpoint.id, paramValues) : EMPTY_SCENE),
		[endpoint, paramValues]
	);
	const scene = useMemo<MapScene>(
		() => ({ features: [...inputScene.features, ...resultScene.features] }),
		[inputScene, resultScene]
	);
	...
	const run = async () => {
		...
		try {
			...
			setResultScene(sceneFromResult(endpoint.id, res.body));
		} catch (err) {
