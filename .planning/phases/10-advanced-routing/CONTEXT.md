...
| D2 | **`/optimize`: OSRM Trip vs VROOM sidecar?** | OSRM `/trip` = zero new infra, but only symmetric TSP (no time windows, capacities, multiple vehicles, or skills). VROOM = real VRP (windows/capacity/multi-vehicle) but a **new sidecar service** to deploy + operate, and it itself calls OSRM `/table`. | **OSRM `/trip` for v1 of this phase**; document VROOM as a fast-follow when a customer needs true VRP. Avoids standing up + securing another service this phase. Confirm you're OK shipping TSP-only `/optimize` first. |
| D3 | **Isochrone hull: `ST_ConcaveHull` vs `ST_ConvexHull` on Neon?** | `ST_ConcaveHull` (PostGIS 3.3+/GEOS 3.11+) gives realistic, "shrink-wrapped" reachability shapes; `ST_ConvexHull` always available but over-estimates reach (rubber-band). | **Probe Neon's `postgis_full_version()` first.** If GEOS ≥ 3.11 → `ST_ConcaveHull(ST_Collect(pts), 0.3..0.1)`. Else fall back to `ST_ConvexHull` and note the accuracy caveat. The plan includes a capability probe task before committing the query. |
